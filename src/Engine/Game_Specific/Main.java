    ...
    public static void main(String[] args) {
        ...
        while(!Display.isCloseRequested()) {
            ...
            if(!spriteHandler.getSpaceShip().dead) {
                ...
                spriteHandler.getSpaceShipShield().translate(spriteHandler.getSpaceShipShield().getPosition().x - spriteHandler.getSpaceShip().getPosition().x, spriteHandler.getSpaceShipShield().getPosition().y - spriteHandler.getSpaceShip().getPosition().y, spriteHandler.getSpaceShipShield().getPosition().z - spriteHandler.getSpaceShip().getPosition().z);
                spriteHandler.getSpaceShipShield().render(shader, camera);
            }
            //Moving the player.
            ...
            //New Velocities
            float newVelocity = 1.0f / spriteHandler.getGameObjects().size() + 0.25f;
            for(int i = 0; i < spriteHandler.getGameObjects().size(); i++) {
                ...
                if(spriteHandler.getGameObjects().get(i) instanceof SpaceInvader) {
                    if(spriteHandler.getGameObjects().get(i).getVelocities().x < 0) {
                        spriteHandler.getGameObjects().get(i).setVelocities(-newVelocity, 0, 0);
                    } else {
                        spriteHandler.getGameObjects().get(i).setVelocities(newVelocity, 0, 0);
                    }
